...
- 二分查找
  - leetcode 540 有序数组中的单一元素 尝试写的更简洁，不用嵌套条件语句（着重思考在target左右的判断、lo和hi的移动策略，可以根据mid和lohi比较判断，也可以从mid周围的情况判断
...
- 动态规划
  ...
  - leetcode 188 只能进行k次的股票交易 分析和309问题的区别
- 搜索
  - leetcode 46 排列  用visited
  - leetcode 77 组合  用start，剪枝
  - leetcode 40 含有相同元素的组合求和 排序和用visited
  - leetcode 131 分割字符串使得每个部分都是回文数 如何制定加入prefix的策略 传入s的时候是传原来的加下标还是传剩下的子串？（如果是要对一长串分子串的任务，最好是传剩下的子串，更清晰）
